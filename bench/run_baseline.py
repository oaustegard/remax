...
# Run as a script (`python bench/run_x.py`) as well as imported as `bench.run_x`:
# in script mode sys.path[0] is bench/ itself, so the sibling modules and the
# in-checkout remax are not importable without this. Kept because the invocation
# contract in the issues and in bench/results/*.md is the bare script path.
if __package__ in (None, ""):  # pragma: no cover - exercised by CLI use only
    _ROOT = Path(__file__).resolve().parents[1]
    sys.path[:0] = [str(_ROOT), str(_ROOT / "src")]

from remax import SignBitQuantizer, StackedSignBitQuantizer
from bench import datasets
from bench.eval import evaluate_quantizer, exact_knn, recall_at_k
...
def _resolve_results_path(out: Optional[str]) -> Path:
    ...
    # src/bench/run_baseline.py → repo_root/bench/results/BASELINE.md
