...
VENDORED COPY. Upstream is the ``gating`` skill's ``scripts/gate.py``; the
skill's own docs say to "import it, or copy the class into your gate script".
It is vendored here so ``bench/gates/*`` runs in CI, where no skill directory
is mounted. ``rotation_identity_gate.py`` still prefers a real skill checkout
(``GATING_SKILL_DIR`` / ``/tmp/gating-skill/scripts`` / ``/mnt/skills/user/
gating/scripts``) and falls back to this file. Stdlib only, on purpose. Do not
add repo-local behaviour here — re-sync from the skill instead.
