...
class StackedSignBitQuantizer:
    ...
    def search_asymmetric(
        ...
        *,
        ...
        codes : np.ndarray, shape (n, self.k * (self.d // 8)), dtype uint8
        ...
        codes = np.ascontiguousarray(codes, dtype=np.uint8)
        if codes.ndim != 2 or codes.shape[1] != self.k * (self.d // 8):
            raise ValueError(
                f"codes shape {codes.shape} incompatible with "
                f"(d={self.d}, k={self.k}); expected "
                f"(n, {self.k * (self.d // 8)})."
            )

        rotated = query @ self._rotation_matrix
        n = codes.shape[0]
        k_eff = min(k, n)
